...
import { View, Text, StyleSheet, TouchableOpacity, FlatList, BackHandler,Dimensions,TextInput } from 'react-native';
import CheckBox from '@react-native-community/checkbox';
import MultiSlider from '@ptomasroos/react-native-multi-slider';

const discountOptions = ['50% and above','60% and above','70% and above','80% and above', '90% and above'];
...
const FilterScreen = ({ onClose, onApply, propSelectedDiscountOptions,propSelectedPriceRange,tabbed,propSelectedStores,propSelectedCategories }) => {
  ...
  const [selectedCategories, setSelectedCategories] = useState(propSelectedCategories);
  ...
  const handleApply = () => {
    onApply(selectedDiscountOptions,priceRange,selectedStores,selectedCategories);
    onClose();
  };
    ...
    return (
      <View style={styles.checkboxContainer}>
        {stores.map((store, index) => {
          const capitalizedStore = store.charAt(0).toUpperCase() + store.slice(1);
          ...
          return (
            ...
            >
              <CheckBox
                value={selectedStores.includes(store)}
                onValueChange={() => toggleStoreSelection(store)}
                tintColors={{ true: '#FF4081', false: '#C0C0C0' }}
              />
  ...
  const renderCategoryItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity style={styles.listItem} onPress={() => toggleCategorySelection(item)}>
        <CheckBox
